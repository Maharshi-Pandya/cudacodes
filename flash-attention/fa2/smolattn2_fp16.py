...
# Load CUDA kernel
smolattn = load(name='smolattn', sources=['build.cpp', 'flash2_fwd_fp16.cu'], extra_cuda_cflags=['-O3'])
...
q = torch.randn((batch_size, n_head, seq_len, head_embd), dtype=torch.float16, device="cuda")
k = torch.randn((batch_size, n_head, seq_len, head_embd), dtype=torch.float16, device="cuda")
v = torch.randn((batch_size, n_head, seq_len, head_embd), dtype=torch.float16, device="cuda")
