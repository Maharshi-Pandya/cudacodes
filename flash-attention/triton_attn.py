...
# Load CUDA kernel
smolattn = load(name='smolattn', sources=['fa1/build.cpp', 'fa1/flash-attn-1.cu'], extra_cuda_cflags=['-O3'])
...
def triton_forward(Q, K, V, causal, dtype=torch.float32):
...
if __name__ == "__main__":
    BATCH_SIZE, NUM_HEADS, SEQ_LEN, HEAD_DIM = 16, 8, 512, 64
    causal = False
    ...
    manual_time, manual_output = benchmark(manual_attn, Q, K, V)
    
    # Benchmark Triton attention
    triton_time, triton_output = benchmark(triton_forward, Q, K, V, causal)
    
    # Benchmark CUDA attention (smolattn)
    smolattn_time, smolattn_output = benchmark(smolattn.fa_forward, Q, K, V)
    
    # Check results match
    match_triton = torch.allclose(manual_output, triton_output, atol=1e-2)
    match_smolattn = torch.allclose(manual_output, smolattn_output, atol=1e-2)
    
    # Print results
    print(f"\nBatch size: {BATCH_SIZE}, Num heads: {NUM_HEADS}, Sequence length: {SEQ_LEN}, Head dims: {HEAD_DIM}\n")

    print(f">> Manual PyTorch Attention: {manual_time:.3f} ms")
    print(f">> Triton (flash attention): {triton_time:.3f} ms")
    print(f">> Smolattn vs. Triton speedup: {triton_time / smolattn_time:.5f}x")
    print(f">> Smolattn vs. Pytorch speedup: {manual_time / smolattn_time:.5f}x")
    print(f">> Smolattn results match: {match_smolattn}")
    print(f">> Triton results match: {match_triton}\n")
