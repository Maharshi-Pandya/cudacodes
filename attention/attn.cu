...
torch::Tensor attention_forward(uint64_t handle, torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    int bs = Q.size(0); int nh = Q.size(1);
    int sl = Q.size(2); int ed = Q.size(3);
    ...
    // scale
    float scale_factor = (1 / sqrt(ed));
    int n_elems_scale = bs * nh * sl * sl;
    dim3 block_dim(1024);
    dim3 grid_dim(CEIL_DIV(n_elems_scale / 4, block_dim.x));
    scale_kernel_inplace<<<grid_dim, block_dim>>>(pre.data_ptr<float>(), scale_factor, bs, nh, sl, ed, n_elems_scale);

    // softmax
    dim3 grid_dim_softmax(bs * nh * sl);
    size_t smem_size = CEIL_DIV(block_dim.x, 32) * sizeof(float);
    softmax_kernel_inplace<<<grid_dim_softmax, ed / 2, smem_size>>>(pre.data_ptr<float>(), bs * nh * sl, sl);
